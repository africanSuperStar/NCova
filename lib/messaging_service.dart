...
class MessagingProvider extends InheritedWidget {
  ...
  static final FirebaseMessaging messanger = FirebaseMessaging();
  static final FirebaseAnalytics analytics = FirebaseAnalytics();

  MessagingProvider({Widget child}) : super(child: child);

  @override
  bool updateShouldNotify(InheritedWidget oldWidget) => true;

  static MessagingProvider of(BuildContext context) => context.dependOnInheritedWidgetOfExactType<MessagingProvider>();
...
class MessagingService {
  ...
  saveDeviceToken(BuildContext context) async {
    // Get the current user
    FirebaseUser currentUser = await RepositoryProvider.of<AuthRepository>(context).getCurrentUser();

    // Get the token for this device
    String fcmToken = await MessagingProvider.messanger.getToken();

    // Save it to Firestore
    if (fcmToken != null) {
      var tokens = Firestore.instance.collection('users').document(currentUser.uid).collection('tokens').document(fcmToken);
  ...
  //   var largeIconPath = await _downloadAndSaveImage('http://via.placeholder.com/128x128/00FF00/000000', 'largeIcon');
  //   var androidPlatformChannelSpecifics = AndroidNotificationDetails(
  //     'media channel id',
  //     'media channel name',
  ...
  //   var lunchBot = Person(name: 'Lunch bot', key: 'bot', bot: true, icon: largeIconPath, iconSource: IconSource.FilePath);
  //   messages.add(Message('Hi', DateTime.now(), null));
  //   messages.add(Message('What\'s up?', DateTime.now().add(Duration(minutes: 5)), coworker));
  //   messages.add(Message('Lunch?', DateTime.now().add(Duration(minutes: 10)), null, dataMimeType: 'image/png', dataUri: imageUri));
  ...
  Future<void> onDidReceiveLocalNotification({@required BuildContext context, int id, String title, String body, String payload}) async {

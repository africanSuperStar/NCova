...
class _LiquidPullToRefreshState extends State<LiquidPullToRefresh> with TickerProviderStateMixin<LiquidPullToRefresh> {
  ...
  Future<void> _dismiss(_LiquidPullToRefreshMode newMode) async {
    ...
    switch (_mode) {
      case _LiquidPullToRefreshMode.done:
        ...
        _ringDisappearController.animateTo(1.0, duration: Duration(milliseconds: widget.springAnimationDurationInMilliseconds), curve: Curves.linear);

        // indicator translate out
        _indicatorMoveWithPeakController.animateTo(0.0, duration: Duration(milliseconds: widget.springAnimationDurationInMilliseconds), curve: Curves.linear);
        _indicatorTranslateInOutController.animateTo(0.0, duration: Duration(milliseconds: widget.springAnimationDurationInMilliseconds), curve: Curves.linear);

        //initial value of controller is 1.0
        await _showPeakController.animateTo(0.3, duration: Duration(milliseconds: (widget.springAnimationDurationInMilliseconds).round()), curve: Curves.linear);

        _radiusController.animateTo(0.0, duration: Duration(milliseconds: (widget.springAnimationDurationInMilliseconds / 5).round()), curve: Curves.linear);

        _showPeakController.value = 0.175;
        await _showPeakController.animateTo(0.1, duration: Duration(milliseconds: (widget.springAnimationDurationInMilliseconds / 5).round()), curve: Curves.easeOut);
        _showPeakController.value = 0.0;

        await _positionController.animateTo(0.0, duration: Duration(milliseconds: _kIndicatorSnapDuration.inMilliseconds * 2));
        break;
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> slivers = List.from(widget.child.buildSlivers(context), growable: true);

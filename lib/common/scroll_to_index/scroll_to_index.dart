...
mixin AutoScrollControllerMixin on ScrollController implements AutoScrollController {
  ...
  double _forecastMoveUnit(int targetIndex, int currentNearestIndex, bool useSuggested) {
    ...
    if (useSuggested && suggestedRowHeight != null) {
      ...
      assert((offsetToLastState?.offset ?? 0) >= 0, "ERROR: %%%%%%%%%%%%%%: $targetIndex, $currentNearestIndex, $alignment, $offsetToLastState, ${tagMap.keys.toList().join(',')}");
  ...
  Future _bringIntoViewportIfNeed(int index, AutoScrollPosition preferPosition, Future move(double offset)) async {
    ...
    if (preferPosition != null) {
      ...
      if (!alreadyInViewport) {
        double value;
        if (preferPosition != null) {
          value = preferPosition == AutoScrollPosition.begin ? begin : preferPosition == AutoScrollPosition.end ? end : _directionalOffsetToRevealInViewport(index, 0.5);
  ...
  AutoScrollPosition _alignmentToPosition(double alignment) => alignment == 0 ? AutoScrollPosition.begin : alignment == 1 ? AutoScrollPosition.end : AutoScrollPosition.middle;
...
class AutoScrollTag extends StatefulWidget {
  ...
  AutoScrollTag({@required Key key, @required this.controller, @required this.index, @required this.child, this.color, this.highlightColor, this.disabled: false}) : super(key: key);
